...
- **Q: How do I update Enfiy Code to the latest version?**
  - A: If installed globally via npm, update Enfiy Code using the command `npm install -g @enfiy/enfiy-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are Enfiy Code configuration files stored?**
  - A: The CLI configuration is stored within two `settings.json` files: one in your home directory and one in your project's root directory. In both locations, `settings.json` is found in the `.enfiy/` folder. Refer to [CLI Configuration](./cli/configuration.md) for more details.
...
- **Error: Command not found (when attempting to run Enfiy Code).**
  - **Cause:** Enfiy Code is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure Enfiy Code installation was successful.
