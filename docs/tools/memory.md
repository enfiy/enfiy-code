...
This document describes the `save_memory` tool for the Enfiy Code.

## Description

Use `save_memory` to save and recall information across your Enfiy Code sessions. With `save_memory`, you can direct the CLI to remember key details across sessions, providing personalized and directed assistance.
...
## How to use `save_memory` with the Enfiy Code

The tool appends the provided `fact` to a special `ENFIY.md` file located in the user's home directory (`~/.enfiy/ENFIY.md`). This file can be configured to have a different name.
...
```
save_memory(fact="The project I'm currently working on is called 'enfiy-cli'.")
```
