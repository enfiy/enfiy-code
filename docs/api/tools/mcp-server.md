# MCP servers with the Enfiy Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with the Enfiy Code.

## What is an MCP server?

An MCP server is an application that exposes tools and resources to the Enfiy Code through the Model Context Protocol, allowing it to interact with external systems and data sources. MCP servers act as a bridge between the Gemini model and your local environment or other services like APIs.

An MCP server enables the Enfiy Code to:
...
- **Access resources:** Read data from specific resources (though the Enfiy Code primarily focuses on tool execution).

With an MCP server, you can extend the Enfiy Code's capabilities to perform actions beyond its built-in features, such as interacting with databases, APIs, custom scripts, or specialized workflows.

## Core Integration Architecture

The Enfiy Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
The Enfiy Code supports three MCP transport types:
...
The Enfiy Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
When the Enfiy Code starts, it performs MCP server discovery through the following detailed process:
...
This comprehensive integration makes MCP servers a powerful way to extend the Enfiy Code's capabilities while maintaining security, reliability, and ease of use.
