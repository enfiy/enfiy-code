...
This document describes the `web_fetch` tool for the Enfiy Code.
...
## How to use `web_fetch` with the Enfiy Code

To use `web_fetch` with the Enfiy Code, provide a natural language prompt that contains URLs. The tool will ask for confirmation before fetching any URLs. Once confirmed, the tool will process URLs through Gemini API's `urlContext`.
