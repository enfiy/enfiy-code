...
export function useModelAutoSwitching({
  ...
  const handleModelError = useCallback(async (
    ...
    try {
      ...
      const fallbackModel = await modelManager.shouldSwitchModel(modelName, error);
  ...
  const checkModelUsage = useCallback(async (modelName?: string): Promise<void> => {
    ...
    try {
      ...
      if (usagePercent >= 95) {
        await handleModelError({ message: 'Usage limit approaching' }, currentModel);
      }
    } catch (error) {
  ...
  const getModelStatus = useCallback(async (modelName?: string) => {
    if (!config) return null;
    ...
    try {
      ...
      return {
        model: currentModel,
        usage,
        usagePercent: usage.limit > 0 ? Math.round((usage.used / usage.limit) * 100) : 0,
