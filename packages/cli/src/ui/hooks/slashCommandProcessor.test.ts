...
describe('useSlashCommandProcessor', () => {
  ...
  let mockEnfiyClient: EnfiyClient;
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getEnfiyClient: () => mockEnfiyClient,
  ...
  describe('/about command', () => {
    ...
    it('should show sandbox-exec profile when applicable', async () => {
      ...
      expect(mockAddItem).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({
          sandboxEnv: 'MacOS Seatbelt (test-profile)',
        }),
        expect.any(Number),
      );
    });
...
`;
  ...
  describe('/tools command', () => {
    ...
    it('should display only Enfiy Code tools (filtering out MCP tools)', async () => {
      // Create mock tools - some with serverName property (MCP tools) and some without (Enfiy Code tools)
      const mockTools = [
    ...
    it('should display a message when no Enfiy Code tools are available', async () => {
  ...
  describe('/mcp command', () => {
    ...
    it('should display a message and open a URL when no MCP servers are configured outside a sandbox', async () => {
      ...
      expect(mockAddItem).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          type: MessageType.INFO,
          text: 'No MCP servers configured. Opening documentation in your browser: https://goo.gle/enfiy-cli-docs-mcp',
        }),
        expect.any(Number),
      );
      expect(open).toHaveBeenCalledWith('https://goo.gle/enfiy-cli-docs-mcp');
      expect(commandResult).toBe(true);
    });
    ...
    it('should indicate when a server has no tools', async () => {
      ...
      const mockServer2Tools: unknown[] = [];
  ...
  describe('/compress command', () => {
    it('should call tryCompressChat(true)', async () => {
      ...
      expect(mockEnfiyClient.tryCompressChat).toHaveBeenCalledWith(true);
      expect(mockAddItem).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({
          type: MessageType.COMPRESSION,
