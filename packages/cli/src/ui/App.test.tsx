...
interface MockServerConfig {
  ...
  getEnfiyMdFileCount: Mock<() => number>;
  setEnfiyMdFileCount: Mock<(count: number) => void>;
  ...
  getAllEnfiyMdFilenames: Mock<() => string[]>;
}
  ...
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        enfiyMdFileCount: opts.enfiyMdFileCount || 0,
        ...
        setUserMemory: vi.fn(),
        getEnfiyMdFileCount: vi.fn(() => opts.enfiyMdFileCount || 0),
        setEnfiyMdFileCount: vi.fn(),
        ...
        getEnfiyClient: vi.fn(() => ({})),
        getCheckpointingEnabled: vi.fn(() => opts.checkpointing ?? true),
        getAllEnfiyMdFilenames: vi.fn(() => ['ENFIY.md']),
...
vi.mock('../config/config.js', async (importOriginal) => {
  ...
  return {
    ...
    ...actual,
    loadHierarchicalEnfiyMemory: vi
      .fn()
...
describe('App UI', () => {
  ...
  const createMockSettings = (
    ...
    const workspaceSettingsFile: SettingsFile = {
      path: '/workspace/.enfiy/settings.json',
      settings: {
        ...settings,
      },
    };
  ...
  it('should display default "ENFIY.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getEnfiyMdFileCount.mockReturnValue(1);
    ...
    expect(lastFrame()).toContain('Using 1 ENFIY.md file');
  });

  it('should display default "ENFIY.md" with plural when contextFileName is not set and count is > 1', async () => {
    mockConfig.getEnfiyMdFileCount.mockReturnValue(2);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
    ...
    expect(lastFrame()).toContain('Using 2 ENFIY.md files');
  });
  ...
  it('should display custom contextFileName with plural when set and count is > 1', async () => {
    ...
    mockConfig.getEnfiyMdFileCount.mockReturnValue(3);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should not display context file message if count is 0, even if contextFileName is set', async () => {
    ...
    mockConfig.getEnfiyMdFileCount.mockReturnValue(0);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should display ENFIY.md and MCP server count when both are present', async () => {
    mockConfig.getEnfiyMdFileCount.mockReturnValue(2);
  ...
  it('should display only MCP server count when ENFIY.md count is 0', async () => {
    mockConfig.getEnfiyMdFileCount.mockReturnValue(0);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
