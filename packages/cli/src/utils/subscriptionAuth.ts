...
export function getSubscriptionAuth(provider: ProviderType): SubscriptionAuth | null {
...
export async function authenticateClaudeSubscription(email: string, password: string): Promise<SubscriptionAuth> {
...
async function simulateClaudeLogin(email: string, password: string): Promise<{
  sessionToken: string;
  tier: 'free' | 'pro' | 'max';
}> {
  ...
  return {
    ...
    tier: email.includes('pro') ? 'pro' : email.includes('max') ? 'max' : 'free'
  };
}
...
export function getProviderDisplayName(provider: ProviderType, authStatus: ReturnType<typeof getAuthStatus>): string {
