...
export const SETTINGS_DIRECTORY_NAME = '.enfiy';
...
export interface Settings {
  ...
  // Allow dynamic properties for testing purposes
  [key: string]: unknown;
}
...
export function loadSettings(workspaceDir: string): LoadedSettings {
  ...
  // Migrate from old .gemini directory if needed
  const oldSettingsPath = path.join(homedir(), '.gemini', 'settings.json');
  if (!fs.existsSync(USER_SETTINGS_PATH) && fs.existsSync(oldSettingsPath)) {
    try {
      ...
      console.log('Migrated settings from .gemini to .enfiy directory');
    } catch (error) {
