...
export class OpenAIProvider extends BaseProvider {
  ...
  protected async performInitialization(config: ProviderConfig): Promise<void> {
    ...
    console.log(`[OpenAI Provider] Initializing with API key: ${config.apiKey.substring(0, 5)}...${config.apiKey.substring(config.apiKey.length - 5)}`);
  ...
  protected convertToStandardResponse(response: OpenAI.Chat.ChatCompletion): GenerateContentResponse {
      ...
      if (delta) {
        ...
        const streamResponse = {
          candidates: [{
            ...
            finishReason: chunk.choices[0]?.finish_reason === 'stop' ? FinishReason.STOP : FinishReason.OTHER,
            index: 0,
            safetyRatings: [],
          }],
          usageMetadata: {
  ...
  protected handleError(error: unknown): Error {
    ...
    console.error('[OpenAI Provider] Raw error:', {
      ...
      stack: 'stack' in errorObj && typeof errorObj.stack === 'string' ? errorObj.stack.split('\n').slice(0, 3) : undefined,
    });

    if (error && typeof error === 'object' && error !== null && 'response' in error) {
      const errorWithResponse = error as { response?: { data?: { error?: { message?: string; code?: string } } } };
      if (errorWithResponse.response?.data?.error) {
        const apiError = errorWithResponse.response.data.error;
        return new Error(`OpenAI API Error: ${apiError.message || apiError.code || 'Unknown error'}`);
      }
    }
    ...
    if (code === 'ENOTFOUND' || code === 'ECONNREFUSED') {
      return new Error(`OpenAI Connection Error: Cannot reach OpenAI API. Check your internet connection.`);
    }
    
    if (code === 'INVALID_API_KEY' || status === 401) {
      return new Error(`OpenAI API Error: Invalid API key. Please check your OPENAI_API_KEY.`);
    }
    
    if (status === 404) {
      return new Error(`OpenAI API Error: Model not found. The model may not be available with your API key.`);
    }
    
    if (status === 429) {
      return new Error(`OpenAI API Error: Rate limit exceeded. Please try again later.`);
    }
