...
export class HuggingFaceProvider extends BaseProvider {
  ...
  convertToHuggingFaceFormat(contents) {
...
// このファイルは自動生成されました。`;
  ...
  extractFileName(text, extension) {
    ...
    const japaneseMatch = text.match(
      /(?:test-debug|テストデバッグ).*?(?:ディレクトリ|directory)/i,
    );
    if (japaneseMatch) {
  ...
  convertToStandardResponse(response, originalText) {
    let content = '';
  ...
  async *simulateStreaming(response) {
