...
// このファイルは自動生成されました。`;
    ...
    convertToStandardResponse(response, originalText) {
        ...
        const standardResponse = {
            ...
            usageMetadata: {
                promptTokenCount: (response && typeof response === 'object' && 'usage' in response) ? response.usage?.prompt_tokens || 0 : 0,
                candidatesTokenCount: (response && typeof response === 'object' && 'usage' in response) ? response.usage?.completion_tokens || 0 : 0,
                totalTokenCount: (response && typeof response === 'object' && 'usage' in response) ? response.usage?.total_tokens || 0 : 0,
            },
    ...
    async generateContent(params) {
        ...
        const supportsChatFormat = ['zephyr', 'Mistral', 'Llama', 'CodeLlama'].some(name => finalModel.includes(name));
