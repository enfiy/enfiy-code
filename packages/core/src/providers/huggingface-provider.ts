    ...
    try {
      if (this.isLocalMode) {
        ...
        const response = await fetch(
          `${this.baseUrl.replace('/models', '')}/health`,
          {
            method: 'GET',
  ...
  private autoDetectFileCreation(text: string, toolCalls: ToolCall[]): void {
    ...
    for (const pattern of codeBlockPatterns) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        console.log('HuggingFace: Found code block:', {
          language,
        ...
        if (!extension) {
          console.log(
            'HuggingFace: No extension found for language:',
            language,
          );
          continue;
        }
  ...
  private detectFileCreationIntent(text: string, toolCalls: ToolCall[]): void {
    ...
    for (const pattern of fileCreationPatterns) {
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        console.log('HuggingFace: Found file creation intent:', {
          fileName,
          extension,
        });
...
// This file was auto-generated.`;
  ...
  private extractFileName(text: string, extension: string): string {
    ...
    const patterns = [
      ...
      new RegExp(
        `([a-zA-Z0-9_/-]+\\.${extension})(?:を|ファイル|作成|という)`,
        'i',
      ),
      new RegExp(
        `(?:作成|つく|make|create).*?([a-zA-Z0-9_/-]+\\.${extension})`,
        'i',
      ),

      // Directory-specific patterns
      new RegExp(
        `(test-debug|src|components|utils|lib|public|assets)/([a-zA-Z0-9_-]+\\.${extension})`,
        'i',
      ),
      new RegExp(
        `(test-debug|src|components|utils|lib|public|assets)/([a-zA-Z0-9_-]+)(?!\\.)`,
        'i',
      ),
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      if (match && match[1]) {
        ...
        if (match[2]) {
          ...
          const result = fullPath.includes('.')
            ? fullPath
            : `${fullPath}.${extension}`;
          console.log(
            'HuggingFace: Extracted filename (with directory):',
            result,
          );
          return result;
        }
        ...
        if (extractedName.includes('.')) {
          console.log(
            'HuggingFace: Extracted filename (with extension):',
            extractedName,
          );
        ...
        console.log(
          'HuggingFace: Extracted filename (added extension):',
          result,
        );
        return result;
      }
      ...
      usageMetadata: {
        promptTokenCount:
          response && typeof response === 'object' && 'usage' in response
            ? (response as HuggingFaceResponse).usage?.prompt_tokens || 0
            : 0,
        candidatesTokenCount:
          response && typeof response === 'object' && 'usage' in response
            ? (response as HuggingFaceResponse).usage?.completion_tokens || 0
            : 0,
        totalTokenCount:
          response && typeof response === 'object' && 'usage' in response
            ? (response as HuggingFaceResponse).usage?.total_tokens || 0
            : 0,
      },
  ...
  async generateContent(params: {
    ...
    const { inputs, messages } = this.convertToHuggingFaceFormat(
      params.contents,
    );
  ...
  protected handleError(error: unknown): Error {
    if (
      ...
      if (errorObj.response?.data?.error) {
        return new Error(
          `HuggingFace API Error: ${errorObj.response.data.error}`,
        );
      }
    }
