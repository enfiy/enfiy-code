...
import { Turn, EnfiyEventType, } from './turn.js';
...
export class EnfiyClient {
  ...
  ${folderStructure}
    ...
    async startChat(extraHistory) {
        ...
        try {
            ...
            const generateContentConfigWithThinking = isThinkingSupported(this.model)
                ...
                : this.generateContentConfig;
            return new EnfiyChat(this.config, this.getContentGenerator(), {
                systemInstruction,
            ...
            await reportError(error, 'Error initializing Enfiy chat session.', history, 'startChat');
    ...
    async generateJson(contents, schema, abortSignal, model = DEFAULT_ENFIY_FLASH_MODEL, config = {}) {
        try {
            const userMemory = this.config.getUserMemory();
            const systemInstruction = getCoreSystemPrompt(userMemory);
            const requestConfig = {
