...
# Tool Execution for Local AI Providers

When using local AI providers (Ollama, HuggingFace) that support text-based tool execution, use the following format to execute tools:

- **File Creation:** [tool_call: WriteFile for file_path: "/path/to/file.html"]
  Then provide the content in a code block:
  \`\`\`html
  <html>
...
- **File Reading:** [tool_call: ReadFile for file_path: "/path/to/file.txt"]

- **File Editing:** [tool_call: EditFile for file_path: "/path/to/file.js"]
  Then describe the changes needed.

- **Shell Commands:** [tool_call: ShellTool for command: "ls -la"]

Always use this exact format with square brackets and the "tool_call:" prefix when you need to execute file operations or shell commands.
...
## Tool Usage
- **File Paths:** Use absolute paths when referring to files with tools like '${ReadFileTool.Name}' or '${WriteFileTool.Name}'. You can also use relative paths, which will be resolved relative to the current working directory provided in the context.
