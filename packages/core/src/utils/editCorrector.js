...
const EditModel = DEFAULT_ENFIY_FLASH_MODEL;
const EditConfig = {
    thinkingConfig: {
        thinkingBudget: 0,
    },
...
export async function correctOldStringMismatch(enfiyClient, fileContent, problematicSnippet, abortSignal) {
    const prompt = `
...
`.trim();
    ...
    try {
        const result = await enfiyClient.generateJson(contents, OLD_STRING_CORRECTION_SCHEMA, abortSignal, EditModel, EditConfig);
        if (result &&
 ...
 */
export async function correctNewString(enfiyClient, originalOldString, correctedOldString, originalNewString, abortSignal) {
...
Return ONLY the corrected string in the specified JSON format with the key 'corrected_new_string'. If no adjustment is deemed necessary or possible, return the original_new_string.
  `.trim();
    const contents = [{ role: 'user', parts: [{ text: prompt }] }];
    try {
        const result = await enfiyClient.generateJson(contents, NEW_STRING_CORRECTION_SCHEMA, abortSignal, EditModel, EditConfig);
        if (result &&
...
export async function correctNewStringEscaping(enfiyClient, oldString, potentiallyProblematicNewString, abortSignal) {
    const prompt = `
...
Return ONLY the corrected string in the specified JSON format with the key 'corrected_new_string_escaping'. If no escaping correction is needed, return the original potentially_problematic_new_string.
  `.trim();
    const contents = [{ role: 'user', parts: [{ text: prompt }] }];
    try {
        const result = await enfiyClient.generateJson(contents, CORRECT_NEW_STRING_ESCAPING_SCHEMA, abortSignal, EditModel, EditConfig);
        if (result &&
