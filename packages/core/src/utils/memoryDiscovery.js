...
async function getGeminiMdFilePathsInternal(
  ...
  for (const enfiyMdFilename of enfiyMdFilenames) {
    ...
    if (debugMode)
      logger.debug(
        `Searching for ${enfiyMdFilename} starting from CWD: ${resolvedCwd}`,
      );
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${enfiyMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
      if (debugMode)
        logger.debug(
          `Global ${enfiyMdFilename} not found or not readable: ${globalMemoryPath}`,
        );
    }
    ...
    while (currentDir && currentDir !== path.dirname(currentDir)) {
      ...
      if (debugMode) {
        logger.debug(
          `Checking for ${enfiyMdFilename} in (upward scan): ${currentDir}`,
        );
      }
      ...
      try {
        ...
        if (potentialPath !== globalMemoryPath) {
          ...
          if (debugMode) {
            logger.debug(
              `Found readable upward ${enfiyMdFilename}: ${potentialPath}`,
            );
          }
        }
      } catch {
        if (debugMode) {
          logger.debug(
            `Upward ${enfiyMdFilename} not found or not readable in: ${currentDir}`,
          );
        }
      }
    ...
    if (debugMode && downwardPaths.length > 0)
      logger.debug(
        `Found downward ${enfiyMdFilename} files (sorted): ${JSON.stringify(downwardPaths)}`,
      );
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllEnfiyMdFilenames()} paths to read: ${JSON.stringify(finalPaths)}`,
    );
  return finalPaths;
}
  ...
  for (const filePath of filePaths) {
    try {
      ...
      if (!isTestEnv) {
        ...
        logger.warn(
          `Warning: Could not read ${getAllEnfiyMdFilenames()} file at ${filePath}. Error: ${message}`,
        );
      }
