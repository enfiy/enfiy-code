...
export async function checkNextSpeaker(chat, enfiyClient, abortSignal) {
    ...
    try {
        const parsedResponse = (await enfiyClient.generateJson(contents, RESPONSE_SCHEMA, abortSignal));
        if (parsedResponse &&
        ...
        console.warn('Failed to talk to Enfiy API when seeing if conversation should continue.', error);
        return null;
    }
}
