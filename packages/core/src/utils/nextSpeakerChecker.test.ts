...
// Mock EnfiyClient and Config constructor
...
describe('checkNextSpeaker', () => {
  ...
  beforeEach(() => {
    ...
    mockEnfiyClient = new EnfiyClient(mockConfigInstance);
    ...
    // EnfiyChat will receive the mocked instances via the mocked GoogleGenAI constructor
    chatInstance = new EnfiyChat(
      mockConfigInstance,
  ...
  it('should return null if history is empty', async () => {
    ...
    expect(mockEnfiyClient.generateJson).not.toHaveBeenCalled();
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockEnfiyClient.generateJson as Mock).mockResolvedValue(mockApiResponse);
    ...
    expect(mockEnfiyClient.generateJson).toHaveBeenCalledTimes(1);
  });
  ...
  it('should return null if geminiClient.generateJson throws an error', async () => {
    ...
    (mockEnfiyClient.generateJson as Mock).mockRejectedValue(
      new Error('API Error'),
    );
  ...
  it('should return null if geminiClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    ...
    (mockEnfiyClient.generateJson as Mock).mockResolvedValue({
