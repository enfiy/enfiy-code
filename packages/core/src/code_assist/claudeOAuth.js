...
export async function getClaudeOAuthClient() {
...
async function authWithClaudeWeb() {
...
async function loadCachedClaudeCredentials() {
  try {
...
async function cacheClaudeCredentials(credentials) {
  const filePath = getClaudeCachedCredentialPath();
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  const credString = JSON.stringify(credentials, null, 2);
  await fs.writeFile(filePath, credString);
}
function getClaudeCachedCredentialPath() {
...
//# sourceMappingURL=claudeOAuth.js.map
