...
const MEMORY_SECTION_HEADER = '## Enfiy Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset ENFIY_MD_FILENAME to its original value after each test
    setEnfiyMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setEnfiyMdFilename', () => {
    it('should update currentEnfiyMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setEnfiyMdFilename(newName);
      expect(getCurrentEnfiyMdFilename()).toBe(newName);
    });

    it('should not update currentEnfiyMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentEnfiyMdFilename(); // Get current before trying to change
      setEnfiyMdFilename('  ');
      expect(getCurrentEnfiyMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setEnfiyMdFilename(newNames);
      expect(getCurrentEnfiyMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllEnfiyMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentEnfiyMdFilename for the default expectation before any setEnfiyMdFilename calls in a test
      const expectedFilePath = path.join(
        '/mock/home',
        '.enfiy',
        getCurrentEnfiyMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
