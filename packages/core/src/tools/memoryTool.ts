...
export const ENFIY_CONFIG_DIR = '.enfiy';
export const DEFAULT_CONTEXT_FILENAME = 'ENFIY.md';
export const MEMORY_SECTION_HEADER = '## Enfiy Added Memories';

// This variable will hold the currently configured filename for ENFIY.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setGeminiMdFilename.
let currentEnfiyMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setEnfiyMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentEnfiyMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentEnfiyMdFilename = newFilename.trim();
  }
}

export function getCurrentEnfiyMdFilename(): string {
  if (Array.isArray(currentEnfiyMdFilename)) {
    return currentEnfiyMdFilename[0];
  }
  return currentEnfiyMdFilename;
}

export function getAllEnfiyMdFilenames(): string[] {
  if (Array.isArray(currentEnfiyMdFilename)) {
    return currentEnfiyMdFilename;
  }
  return [currentEnfiyMdFilename];
...
function getGlobalMemoryFilePath(): string {
  return path.join(homedir(), ENFIY_CONFIG_DIR, getCurrentEnfiyMdFilename());
}
