...
describe('modifyWithEditor', () => {
    ...
    it('should successfully modify content with VSCode editor', async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenNthCalledWith(
        ...
        expect.stringContaining(
          path.join(tempDir, 'enfiy-cli-tool-modify-diffs'),
        ),
        currentContent,
        'utf8',
      );
  ...
  it('should create temp files with correct naming', async () => {
    ...
    expect(oldFilePath).toMatch(/enfiy-cli-modify-test-file\.txt-old-\d+$/);
    expect(newFilePath).toMatch(/enfiy-cli-modify-test-file\.txt-new-\d+$/);
    expect(oldFilePath).toContain(`${tempDir}/enfiy-cli-tool-modify-diffs/`);
    expect(newFilePath).toContain(`${tempDir}/enfiy-cli-tool-modify-diffs/`);
  });
});
